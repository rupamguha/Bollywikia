...
}var a=document.getElementById("trustedsite-tm-overlay");if(!a){a=document.createElement("div");a.id="trustedsite-tm-overlay";
...
a.onclick=function(){TrustedSite.hideverify()};a.style.cursor="zoom-out";document.body.appendChild(a)}var b=document.getElementById("trustedsite-tm-close");
if(!b){b=document.createElement("div");TrustedSite.cleanStyle(b);b.id="trustedsite-tm-close";b.style.backgroundImage="url(https://cdn.ywxi.net/static/img/modal-close.png)";
...
b.onclick=function(){TrustedSite.hideverify()};l.appendChild(b)}if(TrustedSite.is_hidden("#trustedsite-tm-verify")){var k=TrustedSite.window_width();
var g=TrustedSite.window_height();var i=k;if(i>500){i=500}var f=g-60;if(f>480){f=480}l.style.width=i+"px";l.style.height=f+"px";
l.style.top=((g/2)-(f/2))+"px";l.style.left=((k/2)-(i/2))+"px";document.getElementById("trustedsite-tm-overlay").style.display="";
document.getElementById("trustedsite-tm-verify").style.display="";TrustedSite.add_class("body","trustedsite-body-noscroll")
}else{TrustedSite.hideverify()}},hideverify:function(){this.remove_element("#trustedsite-tm-overlay");this.remove_element("#trustedsite-tm-verify");
this.remove_class("body","trustedsite-body-noscroll")},load_conversion:function(){var a;if(TrustedSite.config.platform=="shopify"){if(location.pathname.endsWith("/thank_you")){a=Shopify.checkout.order_id
...
}var d=j.getAttribute("data-color");if(d){f+="&color="+encodeURIComponent(d)}j.style.width=a[0]+"px";j.style.height=a[1]+"px";
...
j.style.backgroundRepeat="no-repeat";j.style.cursor="pointer";j.onclick=function(){if(window.TrustedSite){window.TrustedSite.toggleverify(c)
}else{var k=c=="ts"?"https://www.trustedsite.com":"https://www.mcafeesecure.com";k+="/verify?host="+i;window.open(k)}}},calc_wh:function(b,a,d){var c=b&&b.length==2?b[0]:0;
